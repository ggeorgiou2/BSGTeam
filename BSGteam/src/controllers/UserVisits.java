...
public class UserVisits extends HttpServlet {
	...
	protected void doPost(final HttpServletRequest request,
		...
		try {
			...
			User thisUser = null;
			try {
				thisUser = twitter.showUser(userName);

			} catch (Exception e) {
				request.setAttribute("error",
						"Please check your input, the twitter ID you have entered is incorrect.");
				validUsers = false;
			...
			if (validUsers) {
				...
				// if number of days requested is greater than 0, then the
				// twitter
				// database is
				// queried
				...
				if (days > 0) {
					// gets the date from which previous tweets should be
					// retrieved
					// and
					...
					if (remaining > 1) {
						...
						// creates a foursquare object and inspects the user's
						// tweet
						// for
						// foursquare checkins
						Foursquare foursquare = new Foursquare();
						request.setAttribute("userVisits", foursquare.checkins(
								result, session.getAttribute("clientID")
										.toString(),
							...
							for (Status status : subResults) {
								if (status.getRetweetCount() > 0) {
									System.out.println("sure?"
											+ status.getText());
									contactees.addAll(twitter
											.getRetweets(status.getId()));
								}
							}
						...
						for (Entry<Date, CompactVenue> entry : userVisits
							...
							jena.saveVenue(userName,
									entry.getValue().getName(), photos, entry
											.getValue().getCategories(),
					...
					String customer_key = session.getAttribute("customer_key")
							.toString();
					String customer_sercet = session.getAttribute(
							"customer_secret").toString();

					final String clientID = session.getAttribute("clientID")
							.toString();
					final String clientSecret = session.getAttribute(
							"clinetSec").toString();
					final String redirectURL = session.getAttribute(
							"redirectURL").toString();
					final String accessToken = session.getAttribute(
